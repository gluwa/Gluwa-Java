...
public class GluwaApiSDKImpl implements GluwaApiSDK {
	...
	public GluwaResponse getPaymentQRCode(GluwaTransaction transaction) {
		...
		Header h2 = assignBasicAuth(configuration.getBasic());

		HashMap<String, Object> params = new HashMap<>();
		params.put("Signature", assignSignatureString(configuration.getSign()));
	...
	public GluwaResponse postTransaction(GluwaTransaction transaction) {
		...
		params.put("Signature", assignSignatureForPost(configuration.getSign(), hash));
	...
	public GluwaResponse getListTransactionHistory(GluwaTransaction transaction) {
		...
		Header h1 = new BasicHeader("Accept", "application/json");
		Header h2 = assignSignature(configuration.getSign());
	...
	protected String assignSignatureString(String signature)
	{
		String h2;
		if (signature.isEmpty()) {
			h2 = timestampSignature();
		} else if (signature.equals("null")) {
			h2 = " ";
		} else {
			h2 = getBase64EncodedString(signature);
		}
		return h2;
	}
